...
public class TS_Search_AddedEmployee_inEmployeeList {
	...
	public void Test()
		...
		oPSelFW = new ProlificsSeleniumAPI("PIM Tab", "Search existing Employee in OrangeHRM Application", "Chrome",
				"Windows 10", "QA", "Automation", "TestAutomation", "Regression");
	...
	private static Object[][] asTwoDimensionalArray() throws FileNotFoundException, IOException {
		...
		for (int j = 1; j <= RowCount; j++) {
			XLTestData = excelReader.readExcel("TS_Search_Existing_Employee_" + Integer.toString(j));
	...
	public void orderSearchWCC(HashMap<String, String> XLTestData) throws Exception {
		try {
			oPSelFW.reportStepDetails("Start the New Steps for TestCase",
					"Search existing employee in Employee List in OrangeHRM Application - "
							+ XLTestData.get("Description").toString(),
					"Pass");
			objOrangeHRM.orangeHRMLogin(driver, envTestData, XLTestData, oPSelFW);
			objOrangeHRM.searchEmployee(driver, oPSelFW, XLTestData);
